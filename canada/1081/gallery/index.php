...
function listOutAlbums($con, $selectedID="") {
	$dataTxt="";
	$getAlbums=mysqli_query($con, "select id, title from gallery_name order by title");
		...
		<tr>
			...
			if ($action=="album") {
				if ($sub=="manage") {
					...
					if ($totalrows==0) {							
						...
						<thead>
							<tr>
								<th width=\"20px\"><input type=\"checkbox\" onclick=\"selectAll('dataID[]');\"></th>
								<th scope=\"col\">Title</th>
								<th width=\"90px\">Date</th>
								<th width=\"80px\">Total Images</th>
					...
					$idFromTripProgram = $_GET['trip_id'];
					$titleFromTripProgram = $_GET['title'];
			...
			?>
						...
						<form name="myform" method="post" action="index.php?action=album&sub=add">
							...
							<tr>
								<th>Title: *</th>
								<td><input type="hidden" name="dataTripID" value="<?php print $idFromTripProgram; ?>"><input type="text" name="dataTitle" size="60" maxlength="100" value="<?php print $titleFromTripProgram; ?>"></td>
							</tr>
							<tr>
								<th>Details:</th>
						...
						if ($totalrows==0) {							
							...
							while(($count<$rpp) && ($i<$tcount)) {
								...
								print "<tr $rowAltHead>
									...
									<td><a href=\"../../uploads/gallery/$dataImage\" target=\"_blank\"><img src='$common_path/inc/phpThumb-1.7.11/phpThumb.php?src=/uploads/gallery/$dataImage&w=60&h=60&f=jpeg&zc=T' width='60' height='60' border='0' alt='$title'></a></td>
