			...
			<?php
					...
					$rpp = 8; // results per page
							...
							while(($count<$rpp) && ($i<$tcount)) {
								...
								$dataID=$each['id'];
								$dataName=html_entity_decode($each['name']);
								$dataDesignation=html_entity_decode($each['designation']);
								$dataPosition=$each['position'];
								$dataImage=$each['image'];
								
								print "<tr $rowAltHead>
											<td>$dataPosition</td>
											<td><img src='$common_path/uploads/our_team/$dataImage' height='80px' border='0' alt='$dataTitle'></td>
											<td>$dataName</td>
