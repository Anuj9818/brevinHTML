...
<tr>
	...
	<tr>
		...
		<tr>
			<td>
		<h3>Authorized Management</h3>
		
			<?php
					$query="select * from affiliations order by position asc";
					...
					$rpp = 8; // results per profile
							...
							$tpage = ($tcount) ? ceil($tcount/$rpp) : 1; // total profiles, last profile number
							...
							while(($count<$rpp) && ($i<$tcount)) {
								...
								$dataID=$each['id'];
								$dataLink=html_entity_decode($each['link']);
								$dataPosition=$each['position'];
								$dataImage=$each['image'];

								print "<tr $rowAltHead>
											<td>$dataPosition</td>
											<td><img src='$common_path/uploads/affiliations/$dataImage' width='100px' border='0' alt='$dataTitle'></td>
											<td>$dataLink</td>
							...
							print adminPagination($reload, $profile, $tpage, $adjacents);
						}
					?>
				</td>
			</tr>
