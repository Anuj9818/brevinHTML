...
if ($stm_logged_state==0) {
	...
	if ($_POST['cmd']=="addData") {
		...
		if ($dataName=="" || $dataBatch=="" || $dataPosition=="") {
			print "<script language=\"JavaScript\">
			...
			if ($_FILES['image']['name']=="") {
				...
				if (($_FILES["image"]["type"] == "image/jpg") || ($_FILES["image"]["type"] == "image/jpeg") || ($_FILES["image"]["type"] == "image/png") || ($_FILES["image"]["type"] == "image/gif") || ($_FILES["image"]["type"] == "image/pjpeg")) {
					...
					$uploadImg=move_uploaded_file($_FILES['image']['tmp_name'], "../../uploads/testimonial/tmp_$filename");
					if ($uploadImg==true) {
						
						$sourcefile="../../uploads/testimonial/tmp_$filename";
						$destfile="../../uploads/testimonial/$filename";
						resizeImage_GD2('300', '300', $sourcefile, $destfile);
						unlink("../../uploads/testimonial/tmp_$filename");
			...
			$addData=mysqli_query($con, "insert into testimonial values ('', '$dataName', '$dataBatch', '$dataPosition', '$dataDetails', '$thumbImgFile')");
			if ($addData==true) {
				print "<script language=\"JavaScript\">
