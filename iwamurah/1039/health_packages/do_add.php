		...
		if ($dataTitle=="" || $dataPosition=="" ) {
			...
			if ($_FILES['image']['name']=="") {
				...
				if (($_FILES["image"]["type"] == "image/jpg") || ($_FILES["image"]["type"] == "image/jpeg") || ($_FILES["image"]["type"] == "image/png") || ($_FILES["image"]["type"] == "image/gif") || ($_FILES["image"]["type"] == "image/pjpeg")) {
					...
					$uploadImg=move_uploaded_file($_FILES['image']['tmp_name'], "../../uploads/health_packages/tmp_$filename");
					if ($uploadImg==true) {
						
						$sourcefile="../../uploads/health_packages/tmp_$filename";
						$destfile="../../uploads/health_packages/$filename";
						resizeImage_GD2('370', '370', $sourcefile, $destfile);
						unlink("../../uploads/health_packages/tmp_$filename");
			...
			$addData=mysqli_query($con, "insert into health_packages values ('', '$dataTitle', '$dataPosition', '$dataDetails', '$thumbImgFile')");
			if ($addData==true) {
				print "<script language=\"JavaScript\">
